...
class Phonons(Displacement):
    ...
    def set_constants(self, fc):
        if fc.ndim == 2:
            # Assume single dynamical matrix, wrap it into a batch dimension
            fc = fc[np.newaxis, :, :]  # shape (1, 3N, 3N)

        self.C_N, self.D_N = fc, fc.copy()
    ...
    def read(self, method='Frederiksen', symmetrize=3, acoustic=True,
        ...
        def _load_pair(base):
            # I/O-bound JSON reads in parallel
            fminus = self.cache[base + '-']['forces']
            fplus  = self.cache[base + '+']['forces']
            return fminus, fplus
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            results_iter = ex.map(lambda item: _load_pair(item[4]), work, chunksize=8)

            for (i, a, j, v, base), (fminus_av, fplus_av) in tqdm(
                zip(work, results_iter),
                total=len(work),
                desc="Reading displacements",
                unit="disp",
                leave=True
            ):
    ...
    def get_dos(self, kpts=(10, 10, 10), indices=None, verbose=True):
        ...
        if indices is None:
            ...
            self.ampl_sq = ampl_sq

            masses = self.atoms.get_masses()                          
            mw = ampl_sq * masses[None, None, :]                       
            weights = mw[:, :, indices].sum(axis=2) / mw.sum(axis=2)
            dos = RawDOSData(omegas.ravel(), weights.ravel() / omegas.shape[0])
        return dos

    def partial_dos(self, indices:list):
        """ 
        Return the mass-weighted partial DOS for the specified atoms. 
        Run get_dos() to compute the full DOS and the amplitude squares first, 
        then this function can be used to compute the partial DOS for different sets of atoms 
        without recomputing the full DOS.
        """
        assert self.ampl_sq is not None, "Run get_dos to compute the full DOS first."
        ampl_sq = self.ampl_sq
        masses = self.atoms.get_masses()
        mw = ampl_sq * masses[None, None, :]
        weights = mw[:, :, indices].sum(axis=2) / mw.sum(axis=2)
